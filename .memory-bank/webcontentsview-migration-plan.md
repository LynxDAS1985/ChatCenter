...
**Статус:** 🔴 ПЛАН ЗАКРЫТ как нецелесообразный (25 июня 2026, v1.2.22). Премиса опровергнута живым запуском — см. «ИТОГ РАССЛЕДОВАНИЯ» ниже. Сам план (фазы) оставлен как история.
**Цель (исходная):** включить штатные уведомления мессенджеров (через ServiceWorker), не сломав ничего рабочего.
...
## 🔴 ИТОГ РАССЛЕДОВАНИЯ 25 июня 2026 (v1.2.22) — ГЛАВНОЕ: премиса плана ОПРОВЕРГНУТА

Этот план исходил из фактов F1 и F5, которые **живой запуск опроверг**. Читай это ПЕРЕД остальным планом.

### Опровержение 1 — крах #44934 НЕ исправлен для нашего случая (F5 неверен)
F5 утверждал: #44934 пофикшен в Electron 36-38, на 42 блокер снят. **Реальность**: на Electron 42 in-window child WebContentsView крашит программу **100%** (проверено 3 раза 24 июня, v1.2.20-1.2.21). Журнал каждый раз обрывается на `[wcv-mgr] start-loading` — нативный крах. → блокер НЕ снят.

### Опровержение 2 — SW мёртв НЕ из-за webview, а из-за НАШЕГО кода (вывод F1 неверен)
F1 верно зафиксировал «SW в webview не регистрируется», но НЕВЕРНО вывел «значит нужен WebContentsView». **Реальность (Вариант A, v1.2.22)**: открыли Макс в ОТДЕЛЬНОМ нормальном окне Electron — SW упал с ТОЙ ЖЕ ошибкой `Operation has been aborted`. Причина — мы САМИ убиваем SW + запрещаем уведомления в [sessionSetup.js:24-42](../main/utils/sessionSetup.js#L24). Полная причина — [decisions.md](./decisions.md) ADR «Почему уведомления выключены».

**Вывод**: цель плана (оживить SW-уведомления через WebContentsView) **недостижима сменой движка** — блокер не в движке, а в нашем намеренном решении показывать СВОИ уведомления, а не родные. И сами родные уведомления (в стиле мессенджера) нам **не нужны**.

---

## ✅ Вариант A (v1.2.22) — рабочее решение «Макс в отдельном окне»

Раз in-window WebContentsView крашит, а отдельное окно — нет, сделали безопасный тест-инструмент.

**Что это**: тумблер в Настройках открывает Макс в ОТДЕЛЬНОМ обычном окне Electron (`BrowserWindow` — не `<webview>`, не child WebContentsView). Главное окно не трогается (Макс там остаётся в `<webview>`). Если страница упадёт — закроется **только это окно**, программа выживет.

**Файлы**: [main/handlers/maxTestWindowHandler.js](../main/handlers/maxTestWindowHandler.js) (IPC `max-test:open`/`max-test:close`), [App.jsx](../src/App.jsx) (эффект по флагу), [main.js](../main/main.js) (`initMaxTestWindowHandler`), [SettingsPanel.jsx](../src/components/SettingsPanel.jsx) (тумблер).

**Как запустить (для тестера)**:
1. Запустить приложение (`npm start`).
2. ⚙️ Настройки → тумблер **«🧪 Макс в отдельном окне (тест уведомлений)»** → ВКЛ.
3. Сразу (без перезапуска) откроется отдельное окно с Максом, тот же логин (одинаковый partition).
4. Смотреть журнал `chatcenter.log` — строки с префиксом `[max-test]` (загрузка, SW, уведомления).
5. Выключить тумблер → окно закроется.

**Что показал тест (25 июня)**: окно открывается, Макс грузится, **программа НЕ падает** ✅. SW по-прежнему `Operation has been aborted` — потому что та же сессия с нашими замками (см. выше). Инструмент рабочий; SW глушит наш код, а не webview.

---

## 📊 Плюсы и минусы: перевести ВСЕ веб-приложения на WebContentsView (по фактам, просто)

Вопрос: перевести мессенджеры с `<webview>` на WebContentsView; нативную вкладку (TDLib) оставить как есть (или «тоже перевести»).

**Сначала важное про «API»**: нативная вкладка «ЦентрЧатов» (Telegram через TDLib) — это **React + native API, НЕ веб-страница**. WebContentsView к ней неприменим — там нечего «переводить». Так что «API» в любом случае остаётся как есть. Перевод касается ТОЛЬКО webview-мессенджеров (MAX, WhatsApp, VK, Telegram-web).

**Жёсткая реальность (блокер)**: несколько WebContentsView внутри ОДНОГО окна КРАШАТ Electron на Win11 (#44934, проверено на 42). Значит «перевести все, сохранив текущий вид (вкладки в одном окне)» — **невозможно сейчас**. Работает только: каждый мессенджер в ОТДЕЛЬНОМ окне.

🟢 **Плюсы (если бы перешли — через отдельные окна)**:
- Современно: `<webview>` Electron «не рекомендует» и может убрать в будущем.
- Нет глюков границы webview (разделитель AI-панели перестанет залипать).
- Родные уведомления/SW заработали бы — **НО они нам не нужны** (у нас своя система), так что для нас это НЕ плюс.

🔴 **Минусы (факты)**:
- Краш в одном окне (#44934) → пришлось бы разносить мессенджеры по ОТДЕЛЬНЫМ окнам = жонглировать окнами вместо вкладок. **Большая ломка привычного UI.**
- WebContentsView **непрозрачен** и таким останется (#45105 «not planned») → наши панели/модалки/AI-сайдбар прятались бы ЗА окном → их надо вручную прятать/двигать = сложно и хрупко.
- Нет `destroy()` (#42884) → ручное управление жизнью окон.
- **Огромная переделка**: `webviewSetup.js` (~400 строк), вся инъекция в DOM, перехват уведомлений, навигация — всё завязано на `<webview>`. Дни работы + недели стабилизации.
- Главная выгода (родные SW-уведомления) нам **не нужна** → меняем стабильное на нестабильное почти без пользы.

🟡 **Оставить API (TDLib) как есть** (рекомендуется): риск для Telegram-нативного минимальный — его не трогаем.
🔴 **«Тоже перевести API»**: бессмысленно — TDLib не веб-страница, переводить нечего; только потеряем рабочий native-режим.

### Вывод (просто)
**Сейчас переходить НЕ стоит.** Крах блокирует удобный вид (вкладки), главная выгода нам не нужна (у нас своя модалка), а цена — огромная переделка и потеря стабильности. Правильно: **оставить `<webview>`** для веб-мессенджеров и **TDLib** для Telegram, как сейчас. Пересмотреть — только если (1) Electron починит краш в одном окне И (2) мы решим отказаться от своей системы уведомлений в пользу родных.
