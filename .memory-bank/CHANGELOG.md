...
## 2026-09-19 — новый файл-«дело» `notif-window-input-loss-case.md`

Формат «дело» (case file) для редкого невоспроизводимого глюка: всё про одну беду в одном месте —
симптом, все случаи, факты с источниками, опровергнутое, версии причин, выдержки документации,
что стоит в коде для ловли и как читать журнал, план фаз, журнал наблюдений, критерий закрытия.
Отличие от `mistakes/`: грабля пишется, когда причина ИЗВЕСТНА; дело — пока она ищется.
Указатели: CLAUDE.md (таблица тем), README, `notifications-debug.md`, `mistakes/notifications-ribbon.md`, TODO-55.

## 2026-09-15 — v1.2.462: разгрузка `features.md` (83 → 62 КБ)

- Новый файл `archive/features-v1.2.457-459.md` (22 КБ) — полные записи v1.2.457 (дёргание при
  переключении мессенджеров + ширина списка чатов от окна), v1.2.458 (значок приложения наконец
  вшивается в `.exe`) и v1.2.459 (доводка по придирчивой самопроверке). В `features.md` осталась
  строка-указатель. Зарегистрирован в `archive/README.md`.
- Причина: после записи v1.2.462 активный `features.md` дошёл до 83 КБ из 100 (83 % — выше порога
  предупреждения 80 %). Разгружали НЕ «когда упрёмся», а заранее.
- Новых постоянных файлов памяти не заводили: тема уведомлений уже живёт в `notifications-debug.md`
  (заведён в v1.2.461), туда же дописаны разбор и закрытые пробелы.

## 2026-09-15 — новый файл `notifications-debug.md` (отладка уведомлений по мессенджерам)

**Что**: заведён `.memory-bank/notifications-debug.md` — справочник по отладке уведомлений,
разбитый по мессенджерам (родной Telegram через TDLib, веб-Telegram, МАКС, ВК, WhatsApp, Ozon).

**Зачем**: уведомления рождаются в НЕСКОЛЬКИХ независимых местах — у веб-мессенджера их до трёх
(хук мессенджера, общий быстрый наблюдатель, запасной наблюдатель с «обогащением»). Нигде не было
записано, кто именно шлёт и по какой метке это видно в журнале. Из-за этого одни и те же беды
(дубли, мусорный текст, пустое тело) расследовались с нуля каждый раз. Разбор 2026-09-15 показал
это дважды за один день: три карточки на одно сообщение в МАКС и `[медиа]` в родном Telegram.

**Что внутри**: общая схема рождения уведомления; таблица меток журнала (что грепать в
`chatcenter.log`); по каждому мессенджеру — список источников и известные беды со статусом;
список пробелов «чего не хватает, чтобы разбирать быстрее»; журнал разобранных случаев
(дописывать каждый новый).

**Чем отличается от соседей**: `mistakes/notifications-ribbon.md` — это ЛОВУШКИ окна уведомлений
(«на что наступали»), а новый файл — УСТРОЙСТВО и ОТЛАДКА. Ссылки проставлены в обе стороны.

**Зарегистрирован**: CLAUDE.md (таблица «Файлы по теме задачи»), `.memory-bank/README.md`,
этот журнал.

## 2026-09-11 — разгрузка `mistakes/electron-core.md` (201 КБ → 55 КБ)

**Что**: ловушки за период до 2026-06-24 вынесены в новый `mistakes/electron-core-history.md` (147 КБ).
В активном файле остались свежие (2026-07-30 и позже) + указатель на архивный.

**Зачем**: файл дошёл до 201 КБ при лимите 200 и перестал принимать новые записи — на нём споткнулась
запись ловушки про значок приложения (v1.2.458). До неё файл уже стоял 198,6 КБ (99 %).

**Приём**: тот же, что применён к `notifications-ribbon.md` → `notifications-ribbon-history.md`
в v1.2.78 — история отдельным файлом рядом, в той же папке `mistakes/` (поэтому ссылки внутри
не пересчитывались).

**Зарегистрирован**: CLAUDE.md (таблица `mistakes/`), `common-mistakes.md` (указатель под темой 4),
этот журнал.

## 2026-09-11 — новый файл ловушек `mistakes/app-layout.md`

**Что**: заведён `mistakes/app-layout.md` — ловушки раскладки СОБСТВЕННОГО экрана приложения
(главный ряд, панель ИИ, список чатов, область переписки). Пять ловушек из v1.2.454-457.

**Зачем**: до этого такие ловушки лежали в `ui-components.md`. Рабочий порядок поиска у агента —
`common-mistakes.md` (индекс) → нужный файл из `mistakes/`, плюс `Grep` по `.memory-bank/mistakes/`.
Ловушки в `ui-components.md` в этот путь НЕ попадали и терялись. Ни один из восьми существующих
файлов `mistakes/` тему не покрывал: ближайший `webview-navigation-ui.md` — про UI ВНУТРИ чужих
страниц мессенджеров, а не про наш экран.

**Как разделено**: справочник «как раскладка устроена сейчас» (схема блоков, метки `data-cc-layout`,
как посмотреть замер в журнале) остался в `ui-components.md`; ловушки и правила «как не надо» — в
новом файле, из `ui-components.md` на него стоит ссылка.

**Зарегистрирован**: CLAUDE.md (таблица `mistakes/`), `common-mistakes.md` (тема 2d),
`.memory-bank/README.md`, этот журнал.

**Заодно**: `common-mistakes.md` упирался в свой предел 10 КБ (10236 байт из 10240) — в индекс нельзя
было добавить ни строки. Ужаты описания двух тем (сами темы сохранены, ссылки целы), место под новую
тему освобождено.

## 2026-06-24 — CLAUDE.md облегчён 99→56 КБ (v1.2.19)

**Зачем:** CLAUDE.md грузится в контекст КАЖДУЮ сессию, вырос до 99 КБ → постоянный расход.

**Что сделано:**
- Авто-таблица «Структура памяти» (~120 строк, список всех файлов памяти с размерами) вынесена из CLAUDE.md в новый **`STRUCTURE.md`**. Регенератор `scripts/regen-claude-structure.sh` перенацелен с `CLAUDE.md` на `.memory-bank/STRUCTURE.md`. В CLAUDE.md — короткий указатель.
- Дубль changelog-подвала (4 гигантские строки «Последнее/Предыдущее обновление» ~35 КБ, дублировали `features.md`) убран → 1 строка-сводка + указатель на `features.md`.
- Итог: CLAUDE.md **99 КБ → 56 КБ (−43%)**, информация не потеряна.
- Новые файлы памяти этой сессии: `webcontentsview-migration-plan.md` (план миграции ради SW-уведомлений), `STRUCTURE.md` (авто-структура).

**Синхронизировано:** CLAUDE.md (указатель + троблшутинг), README.md, этот CHANGELOG, регенератор-скрипт.
...
1. Перемотка не работает на полностью скачанных видео
2. Хочется смотреть пока грузится
3. Некоторые видео падают с DECODER_ERROR_NOT_SUPPORTED (codec issue)

### Корневая причина seek bug

`ccMediaProtocol.handle` использовал `net.fetch('file://...')` — в текущей версии Electron этот вызов не пробрасывает Range header корректно для `file://` URLs. `<video>` element НЕ получал `Accept-Ranges: bytes` в response → seek не работал.

### Решения

**Manual Range support**: парсим `Range: bytes=N-M` сами, `fs.createReadStream({start,end})`, `Readable.toWeb(stream)` → Web ReadableStream, возвращаем 206 Partial Content с `Content-Range` + `Accept-Ranges`. Для запросов без Range — 200 OK с `Accept-Ranges: bytes`.

**Progressive playback**: `downloadFile` резолвится early при `downloaded_prefix_size >= 256 KB` (достаточно для MP4 metadata + старта). UI открывает video-player с cc-media URL, `<video>` начинает играть, TDLib докачивает в фоне. cc-media handler читает file size динамически через `fs.statSync`. Юзер играет с начала → нормально. Сидает за пределы скачанного → buffer wait. После завершения — full seek работает.

**Codec error UX**: при `MediaError.code === 4` или `DECODER_ERROR`/`kUnsupportedConfig` — рендерим дружелюбный UI вместо технической ошибки + кнопка «🎬 Открыть во внешнем плеере». Новый IPC `video:open-external` конвертирует cc-media URL в OS path и вызывает `shell.openPath` → Windows откроет в VLC/Movies&TV.

### Урок

Полагаться на `net.fetch('file://...')` для Range support — рискованно (зависит от версии Electron). Manual Range handling через `fs.createReadStream({start, end})` — надёжно и работает одинаково во всех версиях.

Также: для всех custom protocol handlers где UI делает streaming (`<video>`, `<audio>`) — НУЖНО возвращать `Accept-Ranges: bytes` даже для 200 OK ответов, чтобы `<video>` знал что seek потенциально поддерживается.
...
1. **api.md:37** — устаревший shape `tg:get-accounts` с `name/phone` (после v0.89.4 фикса #7 этих полей нет). Документация приведена в соответствие.
2. **applicationVersion drift** — `main.js` не передавал реальную версию, fallback `'0.89.2'` в startup/auth → TDLib записывал «ChatCenter 0.89.2» в session-БД для новых login'ов. Теперь `main.js:230` передаёт `app.getVersion()` (Electron API из `package.json`) → автообновление.

**Итог 4 раундов**: TDLib миграция завершена с зрелой защитой от регрессий. Invoke и emit стороны IPC покрыты контракт-тестами. 4-й раунд подтвердил отсутствие функциональных проблем — оставалась только cosmetic drift в docs/version-string.
...
## Более старые записи — в архиве

История изменений структуры памяти до этой отметки вынесена в
[archive/memory-changelog-old.md](archive/memory-changelog-old.md)
(разгрузка 2026-09-16: файл дорос до 96 КБ при лимите 100 КБ).
