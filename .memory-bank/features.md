...
В [`unreadAutoPrefetch.test.cjs`](src/__tests__/unreadAutoPrefetch.test.cjs) добавлены статические проверки на критичные интеграции, которые **с большой вероятностью пострадают при внедрении виртуализации**:
...
- `main/native/telegramMessages.js`: добавлен параметр `afterId` в [`tg:get-messages`](main/native/telegramMessages.js) и [`tg:get-topic-messages`](main/native/telegramMessages.js). Маппится в MTProto `min_id` + `offset_id=0` + `add_offset=-limit`. При `afterId` бэкенд эмитит `tg:messages` с флагом `appendNewer: true`.
