...
## Текущая версия: v0.86.2 (13 апреля 2026)
...
### v0.86.2 (13 апреля 2026) — REVERT: убран WhatsApp sidebar watcher (ловушка 62)
- **Sidebar watcher УБРАН** — давал больше фантомов чем пользы
- Логи 06:29:21 показали: WhatsApp ШЛЁТ Notification API при активной вкладке ("Выя", "Ауыч")
- Фантомы приходили из `span[dir]` в sidebar — попадали в текст SVG-иконок (`ic-expand-more`, `status-dblcheck`)
- Реальные сообщения через `__CC_NOTIF__` всегда работали — sidebar не нужен
- Ловушка 62 задокументирована: НЕ строить решения на ошибочных предположениях, проверять логи
