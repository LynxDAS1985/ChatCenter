...
- ❌→🔧 **Видео в отдельном окне — НЕ РАБОТАЛО** (v0.87.36-38: пустой плеер). Причина: ready-to-show → send IPC ДО выполнения JS → потеря события. Фикс v0.87.38: did-finish-load + 200мс delay. Ожидает подтверждения.
