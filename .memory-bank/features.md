...
### v1.2.0 — 🌉 AI Bridge RELEASE (Этапы 1-9 завершены, документация финализирована)

**Главное**: программа теперь умеет получать ответы AI **тремя способами** через **единый интерфейс**:
1. **Локальный Ollama** — бесплатно, без интернета, без ключей.
2. **API 4 провайдеров** — Anthropic / OpenAI / DeepSeek / ГигаЧат (ключи юзера из настроек).
3. **Веб-интерфейс AI сайтов** — chat.openai.com / chat.deepseek.com / claude.ai / giga.chat (DOM injection через preload + hook, юзер залогинен сам).

Плюс **fallback chain** — авто-переключение на резерв если основной упал.

**Что нового в v1.2.0** (по сравнению с предыдущими этапами):

- 📘 [`.memory-bank/ai-bridge.md`](.memory-bank/ai-bridge.md) — единая документация AI Bridge:
  - Архитектурная диаграмма (renderer → IPC → main → 3 bridges → внешние AI)
  - Описание всех 9 компонентов (contracts / router / 3 bridges / preload / 4 hooks / 2 UI компонента / fallback chain)
  - **Таблица 11 IPC каналов** с направлением и payload
  - **Таблица 12 кодов ошибок** + retryable flag + fallback правила
  - Как юзер использует через UI «🤖 Проверка AI»
  - Как разработчик использует программно (single mode + fallback chain)
  - Безопасность: ключи в main / sandbox / partition / нет console.\*
  - Покрытие тестами ~200 unit-тестов
  - История этапов v1.1.7 → v1.2.0

- 📑 [`CLAUDE.md`](CLAUDE.md) — таблица «Файлы по теме задачи» дополнена строкой про AI Bridge:
  ```
  🌉 AI Bridge (v1.2.0) → ai-bridge.md
  ```
  Это означает: при изменении `main/ai/bridge/*` или `main/preloads/hooks/ai/*` — читать `ai-bridge.md` обязательно.

- 🎯 **Бамп версии 1.1.18 → 1.2.0** (minor) — финал mini-релиза:
  - 9 этапов AI Bridge закрыты.
  - 1 этап разбиения файлов (v1.1.9) для запаса лимитов.
  - 2 UI-итерации (v1.1.14/v1.1.15) — финальная кнопка в AISidebar.
  - +~200 unit-тестов покрытия Bridge.

### Финальная архитектура

```
RENDERER:                          MAIN:
sendQuestion()                     handleSend (ai-bridge:send IPC)
   │                                  │
   │                                  ├── chain[] → createFallbackChain (Этап 9)
   │                                  ├── mode=local → createLocalBridge (Этап 2)
   │                                  ├── mode=api → createApiBridge (Этап 3) → callProvider → 4 API
   │                                  └── mode=webui → createWebUiBridge (Этап 4)
useAiWebviewBridge (Этап 7)                              │
   ↓                                                     │ webContents.send → preload
<webview preload=ai-monitor>     ←──── register-webview ↑
   ↓
ai-monitor.preload (Этап 4)        →  загружает hook
   ↓
hook (openai/deepseek/anthropic/   →  DOM injection → ждёт ответ → postMessage
       gigachat).hook.js (5-6)
```

### Что юзер видит в UI

1. **🤖 Проверка AI** (v1.1.15) — кнопка в шапке AISidebar. Окно с выбором режима, провайдера, текстом вопроса, кнопкой «📤 Спросить». Все этапы в «📒 Логи ChatCenter».
2. **🔧 Настройка селекторов AI сайтов** (Этап 8) — внутри «🤖 Проверка AI» при mode='webui'. Редактирование 4 CSS селекторов на каждый из 4 провайдеров. Применяется при следующем вызове.

### Что юзер видит в логах

При активном использовании AI Bridge в `chatcenter.log`:
- `[ai-bridge-check] start mode=local providerId=...` — отправка вопроса
- `[ai-bridge-check] result ok=true latencyMs=...` — успех
- `[ai-webview-bridge] detected provider=openai` — Этап 7 регистрация
- `[ai-hook:openai] submit clicked` — Этап 5 инъекция вопроса в сайт
- `[ai-hook:openai] ответ готов (245 chars)` — Этап 5 ответ получен
- `[ai-selectors-editor] saved providerId=openai fields=input,submitButton` — Этап 8 настройка
- `[ai-monitor.preload] hook инжектирован, provider=...` — Этап 4 загрузка preload

### Что НЕ входит в v1.2.0

См. раздел «🔜 Что НЕ входит в v1.2.0» в [`ai-bridge.md`](.memory-bank/ai-bridge.md):
- AI Agent + Bridge composition
- UI «авто-резерв» (galочка в AiBridgeCheck для построения chain)
- AI auto-reply через Bridge
- Native API не-Telegram мессенджеров
...
| Lint | 0 warnings |
| Vitest | 1753/1753 ✅ |
| File size limits | 444/444 ✅ |
| Check-memory | ✅ |
| AI Bridge тесты | ~200 unit |
| Покрытие новых модулей | ~98% |

### Релиз готов
- ✅ Этапы 1-9 закрыты с тестами + документацией
- ✅ Единый IPC канал `ai-bridge:send` с 3 mode + fallback chain
- ✅ UI 🤖 Проверка AI + 🔧 Настройка селекторов в AISidebar
- ✅ Документация `ai-bridge.md` + CLAUDE.md ссылка
- ✅ ~200 unit-тестов покрытия Bridge
- ✅ Безопасность: API ключи только в main, sandbox для preload, app:log для всех логов

---

### v1.1.16 – v1.1.18 — заархивированы

AI Bridge Этапы 7-9: подключение webview к WebUI Bridge через preload + registerWebview (v1.1.16) + UI редактор кастомных CSS селекторов (v1.1.17) + fallback chain auto-резерв (v1.1.18). Все объединены в финал v1.2.0. Подробно: [`archive/features-v1.1.16-1.1.18.md`](./archive/features-v1.1.16-1.1.18.md). Единая документация: [`ai-bridge.md`](./ai-bridge.md).
