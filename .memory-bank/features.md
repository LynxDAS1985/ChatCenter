...
- ✅ **Unread-счётчик в списке чатов — РАБОТАЕТ** (подтверждено — «ЗАФИКСИРУЙ ЧТО сделано»)
- ❌ **Стрелка «↓ к непрочитанному» — НЕ ВИДНА** на экране. Гипотеза: `.native-scroll-bottom-btn` с `position: absolute` находится ВНУТРИ scrollable контейнера → при скролле уезжает вместе с контентом; нужен вынос в не-скролящийся parent или `position: sticky`.
- ❌ **Видео не играет в отдельном окне** — окно открывается, но `<video>` показывает пустую рамку. Гипотеза: CSP / cc-media:// Range в отдельном BrowserWindow, либо файл не скачался. Нужны варианты — **юзер хочет inline в чате**, с опциональным expand в окно.
- ❌ **Открытие старых чатов**: первые ~минуту показывается «Нет сообщений», потом чат появляется. Нужен индикатор загрузки (skeleton / spinner / shimmer).
