...
### v0.44.2 (6 марта 2026) — Автозагрузка WebView при старте
- **Фикс**: WebView не загружались до перехода на вкладку — `display: none` блокирует загрузку `<webview>` в Electron. Заменено на `visibility: hidden` + `zIndex` — все WebView теперь загружаются сразу при старте приложения.
