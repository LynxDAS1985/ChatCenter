...
### v0.73.3 (17 марта 2026) — Overlay badge: Canvas УДАЛЁН, BGRA 32×32 с шрифтом 5×7
- **Canvas удалён полностью** — не работал ни в каком варианте (offscreen, hidden BrowserWindow, renderer). Все размеры (64, 256, 32) показывали неправильные цифры.
- **Новый механизм**: BGRA buffer 32×32 напрямую в main.js. Шрифт 5×7 (вместо 3×5) — цифры "3" и "5" чётко различимы. Масштаб ×2 для двузначных чисел, ×4 для однозначных. Чёрная обводка 1px для контраста.
- **Удалено**: overlayCanvasRef, overlayDataURL, Canvas-рендер из App.jsx.
