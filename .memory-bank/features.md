...
- ❌→✅ **Стрелка «↓» — ПОЧИНЕНА** (v0.87.36: вынесена из scroll-контейнера) — ожидает подтверждения
- ❌→✅ **Видео inline — ПЕРЕДЕЛАНО** (v0.87.36: играет в чате, ⛶ для отдельного окна) — ожидает подтверждения
- ❌→✅ **Загрузка старых чатов — Skeleton+Shimmer** (v0.87.36: кэш+skeleton) — ожидает подтверждения
