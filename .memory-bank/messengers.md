...
### MAX sidebar fallback — правило v1.2.33

`max-sidebar` читает список чатов, а не message bubble. Поэтому изменение preview само по себе не является доказательством входящего сообщения: preview меняется и при собственной отправке, и при переотрисовке старой строки. С v1.2.32 sidebar fallback отправляет `__CC_NOTIF__` только когда у строки вырос unread-бейдж. Если preview изменился без роста unread, baseline обновляется, но уведомление не показывается. Реальные Notification API/SW/active observer пути остаются основными.

Уточнение v1.2.33: первый неизвестный sidebar-row с unread можно показать только если прямо перед ним вырос title-unread MAX (`document.title`, например `1 непрочитанный чат`). Это закрывает случай, когда первое реальное сообщение видно сначала только как рост счётчика, но не возвращает голый title-fallback: без sidebar sender/body/avatar/unread уведомление не создаётся.
