...
### TODO-scroll-jump: чат «прыгает вверх» при возврате во вкладку
Из первого сообщения сессии («полистал чат, перешёл на другой, вернулся — перекидывает вверх»). Не разобран — нужно уточнить экран: MAX `<webview>` или нативный инбокс (InboxMode)? Механизм и фикс разные.
