...
**Дата**: 2026-06-09
**Версия проекта**: v0.98.0 (Phase 2 готов)
**Активная фаза**: Phase 3 (начинается — UI агента)
**Последний завершённый milestone**: M2.8 Security tests

### Manual checks результаты (2026-06-09)

- ✅ **Тест 5** WebView мессенджеры + AI WebView mode не сломаны — юзер подтвердил «всё хорошо»
- ⏳ Тест 4 AI агент через консоль — будет проверка после Phase 3 (UI кнопка)
- ⏳ Тест 2 стикеры, Тест 3 возврат в чат — пропущены юзером
- ✅ Cross-tab уведомления работают (подтверждено логом)

### Уточнение по «уведомление не приходит» (зафиксировано)

Юзер сообщил что уведомление не приходит в ЦентрЧатов при отправке с webview-вкладки.
Расследование (см. `chatcenter.log` 13:42:29): `isOutgoing=true` — сообщение от того же
Telegram-аккаунта. TDLib spec правильно ставит is_outgoing=true для cross-session.
Это **не баг**, это spec поведение. Зафиксировано как **ловушка #30** в
[.memory-bank/mistakes/notifications-ribbon.md](../mistakes/notifications-ribbon.md).
