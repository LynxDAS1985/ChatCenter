...
```
## 2026-06-29 — контроль после v1.2.27: title-фантомы MAX стали тише, но найден отдельный путь `__CC_NOTIF__`
...
**Новая отдельная проблема:** пользователь отправлял/пересылал сообщения сам, но приложение показало уведомление как входящее. В логе это пришло не через `MAX title-fallback`, а через штатный перехват уведомлений MAX:
...
**Почему это не тот же баг:** v1.2.27 отключил создание ribbon из `page-title-updated`, но не блокировал реальные `__CC_NOTIF__`. Это было сделано специально, потому что реальные короткие сообщения MAX приходят именно через `__CC_NOTIF__`. Поэтому текущий фантом не доказывает возврат старого title-fallback; он доказывает, что сам MAX Web может вызвать browser Notification и для события, которое пользователь воспринимает как свое исходящее/пересланное.
...
**Текущий вывод:** v1.2.27 помогла именно против фантомов от `title/unread`. Новая проблема находится в доверии к `__CC_NOTIF__` от MAX без проверки направления сообщения. Это отдельный баг-класс: "MAX Notification API может принести уведомление на свое исходящее/пересланное событие".
