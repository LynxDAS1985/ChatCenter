# Ловушки Electron-ядра — АРХИВ СТАРЫХ (до 2026-06-24)

> Вынесено из [electron-core.md](electron-core.md) 2026-09-11: активный файл дошёл до 201 КБ
> (лимит 200 КБ) и не принимал новых записей. Приём тот же, что применён к
> `notifications-ribbon.md` → `notifications-ribbon-history.md` в v1.2.78.
> Здесь — ловушки Electron/IPC/настроек/AI ЗА ПЕРИОД ДО 2026-06-24. Свежие (2026-07-30 и позже)
> остались в активном файле. Ссылки внутри не менялись: файл лежит в той же папке `mistakes/`.
...
### ❌ 🔴 Путь preload с расширением `.cjs` вместо `.mjs` → окно пустое ТОЛЬКО в установленной версии (v1.2.318)

**Симптом**: окно «Логи ChatCenter» в УСТАНОВЛЕННОМ приложении вечно висит на «Загрузка …» / «0 записей». В dev (запуск из терминала) всё работает. Пользователь жаловался: «почему логи пустые??? это всё установленное приложение».

**Корень** (по коду): `trayManager.js` строил путь preload для prod как `../preload/log-viewer.cjs`, а electron-vite собирает ВСЕ preload как `.mjs` (`out/preload/log-viewer.mjs`). Файла `.cjs` в сборке НЕТ → preload не грузится → `contextBridge` не выполняется → `window.logViewer` отсутствует → окно не получает содержимое лога. В dev путь указывает на ИСХОДНИК `main/preloads/log-viewer.preload.cjs` (он реально `.cjs`), поэтому dev не ломается — баг виден только в упакованной версии.

**Почему не поймали раньше**: preload-скрипт, который не загрузился, НЕ роняет окно и НЕ пишет в наш `chatcenter.log` (ошибка уходит в стандартный stderr Electron, а не в файл). Окно просто тихо остаётся без моста. Сам файл лога при этом пишется нормально — ломается только ПОКАЗ.

**Правильно**: у ВСЕХ рабочих окон prod-путь preload = `.mjs` (`notification.mjs`, `pin.mjs`, `pin-dock.mjs`, `monitor.mjs`, `photoViewer.mjs`). Любое НОВОЕ окно с preload должно в prod указывать на `.mjs`. Проверка: `grep -rE "\.\./preload/[a-z-]+\.(cjs|mjs)" main/` — все prod-пути должны быть `.mjs`, ни одного `.cjs`.

**Профилактика (сделано)**: окно логов теперь при открытии САМО запрашивает лог (`window.logViewer.readLog()`), а если моста нет — показывает честное «Не удалось подключить мост логов», а не вечную «Загрузку …». Так следующий такой сбой будет виден сразу, а не как «пустое окно».

**Урок**: `.cjs`/`.mjs` в путях preload — классическая «тихая дыра» dev-vs-prod. Расширение в prod-пути ВСЕГДА сверять с тем, что реально кладёт сборка в `out/preload/`. Связано с [[feedback_user_runs_dev_terminal]].

---

### ❌ 🟡 Наш `<webview>` блокируется строгими антибот-сайтами (Ozon): подделанный UA + глушение Service Worker (v1.2.320)

**Симптом**: Ozon-кабинет (`seller.ozon.ru`), добавленный как веб-вкладка, показывает «Похоже, нет соединения / Выключите VPN» (код `fab_chlg…`) даже БЕЗ VPN. В обычном Chrome на той же машине без VPN — открывается. То есть виновата не сеть, а поведение нашего `<webview>`.

**Корень** (`main/utils/sessionSetup.js`): `setupSession` для всех веб-сессий делал две «ненормальные» для обычного браузера вещи, которые ловит антибот:
1. **Подделка UA под фикс. `Chrome/131.0.0.0`**, тогда как реальный движок — Chromium из текущего Electron (новее 131). Антибот сверяет UA-строку с client hints (`Sec-CH-UA`/`navigator.userAgentData`, которые Electron заполняет РЕАЛЬНОЙ версией) → нестыковка = «бот».
2. **Глушение Service Worker** (`clearStorageData({storages:['serviceworkers','cachestorage']})` + убийца по `running-status-changed`). У сайтов-SPA (Ozon) SW держит соединение; убили → сайт показывает «нет соединения».

**Фикс (точечный, только Ozon)**: `setupSession(ses, {url})` → для `*.ozon.ru` (разбор `new URL().hostname`) UA берётся с реальной версией (`process.versions.chrome` — совпадает с client hints) и SW НЕ глушится. Остальные мессенджеры не тронуты. Диагностика: лог `[Session] setup … strict=… ua=… sw=…`.

**✅ РЕЗУЛЬТАТ (2026-09-01, подтверждено пользователем)**: фикс СРАБОТАЛ — после v1.2.320 Ozon-кабинет во встроенном окне открывает штатную страницу входа «OZON Seller» вместо блока. То есть для Ozon хватило снять ДВЕ наши помехи (UA + SW); официальный API НЕ понадобился (по крайней мере до этапа входа).

**✅ ВТОРОЙ СЛОЙ (v1.2.322, 2026-09-01, подтверждено)**: после входа страница Ozon грузилась и чаты были видны, НО сама показывала тосты `Request failed 403 /api/v2/resolve` — в обычном Chrome их НЕТ, а в нашем окне ЕСТЬ. Корень: v1.2.320 выровнял UA-строку, но `Sec-CH-UA` (client hints) у Electron по умолчанию содержат бренд **`Electron`** — строгий антибот Ozon ловил его на глубоком запросе. Фикс v1.2.322: в strict-сессии через `webRequest.onBeforeSendHeaders` переписываем `Sec-CH-UA`/`Sec-CH-UA-Full-Version-List` под обычный Chrome (Chromium + Google Chrome, без Electron; чистая `chromeClientHints(process.versions.chrome)`). После полного перезапуска ошибки ПРОПАЛИ. **Урок доп.**: подделки UA-строки МАЛО — строгие антибот-сайты сверяют и `Sec-CH-UA` client hints; бренд `Electron` там надо убирать (окно — настоящий Chromium, это честная нормализация). Диагностика — лог `[ozon-req] … Sec-CH-UA было=… → стало=…`.

**Уроки**: (1) поддельный UA ДОЛЖЕН совпадать с реальной версией движка — иначе строгие сайты видят нестыковку; хардкод версии Chrome устаревает с каждым апдейтом Electron. (2) Глобальное глушение Service Worker ломает сайты, которым SW нужен для связи — включать точечно. (3) `process.versions.chrome` есть только в Electron (в обычном Node — `undefined`, нужен fallback). (4) Диагностика dev-vs-prod: воспроизводить в обычном браузере на той же машине/IP — если там работает, а во встроенном окне нет, вина почти всегда в НАШИХ модификациях сессии (UA/SW/заголовки), а не в сети. (5) Первый шаг для «строгого» сайта — дать реальный UA и НЕ трогать его SW.

**✅ ТРЕТИЙ СЛОЙ (v1.2.323-325, 2026-09-01) — почему хук-разведчик Ozon не запускался, и ложный диагноз**:
Хук мессенджера впрыскивается ДВУМЯ путями: (а) `monitor.preload.cjs` вставляет ВСТРОЕННЫЙ `<script>` с кодом хука; (б) запасной путь `webviewSetup.js:361-371` — `app:read-hook` отдаёт код, `el.executeJavaScript(code)` исполняет (МИНУЯ CSP). У строгих сайтов (Ozon) **CSP `script-src` блокирует встроенный `<script>`** (путь «а»), а `sessionSetup` вырезает из CSP только `frame-ancestors`, НЕ `script-src`. Значит для таких сайтов работает ТОЛЬКО путь «б» (executeJavaScript). НО путь «б» берёт тип через `detectMessengerType(url)` — и если новый мессенджер туда НЕ добавлен, вернётся `unknown` → `app:read-hook('unknown')` → не тот файл → хук не впрыснется НИ одним путём. **Фикс**: добавить сайт в `detectMessengerType` (для Ozon — `ozon.ru → 'ozon'`, v1.2.325). **Урок про диагноз (мой промах)**: сначала я решил, что виноват путь `out/preload/hooks` vs `out/preloadS/hooks`, и «починил» `monitor.preload` многопутёвостью (v1.2.324). Ревью показало: в DEV монитор грузится из ИСХОДНИКА (`mainIpcHandlers.js:247` → `__dirname=main/preloads`), где файл хука УЖЕ был → та правка не могла быть причиной. **Мораль**: прежде чем «чинить» путь — проверь, откуда реально грузится модуль в dev vs prod (`isDev`-ветка), и есть ли ВТОРОЙ путь инъекции (executeJavaScript), который у строгих сайтов и есть единственный рабочий.

---

### ❌ 🟡 `innerText` в массовом обходе DOM (хук/разведчик) подвешивает страницу — reflow-шторм (v1.2.327)

**Симптом/риск**: разведчик Ozon (`ozon.hook.js`) для сбора структуры читал текст элементов через `el.innerText` в проходах по ТЫСЯЧАМ элементов (`findUnread` до 6000, `findInputs` до 4000) ×5 сканов. `innerText` (в отличие от `textContent`) — **форсирует пересчёт раскладки (reflow)** на КАЖДОМ вызове (браузер обязан вычислить «видимый» текст с учётом CSS). Итого до ~50 000 принудительных reflow → страница может подвисать/дёргаться, особенно пока пользователь в ней работает.

**Факт уровня 1 (MDN)**: чтение `innerText` вызывает reflow; `textContent` — нет (просто конкатенация текстовых узлов).

**Фикс (v1.2.327)**: в массовых обходах DOM использовать `textContent`, а НЕ `innerText`. `getComputedStyle`/`getBoundingClientRect` (тоже форсируют reflow) звать ТОЛЬКО на нескольких целевых элементах (≤7 за скан), а не на тысячах.

**Урок**: для впрыснутых скриптов-разведчиков/сторожей, которые сканируют весь DOM — `textContent` по умолчанию; `innerText`/`getComputedStyle`/`getBoundingClientRect` — точечно и осознанно (каждый = reflow). Иначе тяжёлый хук делает WebView-страницу лагающей. Связано с разведкой Ozon ([[code-todo]] TODO-33).

---

### 🟢 Наш намеренный SW-kill СЫПЛЕТСЯ ложной «ошибкой» в журнал каждые 30с (МАКС/ВК/WhatsApp) — фильтровать шум (v1.2.332, 2026-09-01)

**Симптом**: в `chatcenter.log` у МАКС (и потенциально ВК/WhatsApp/веб-Telegram) каждые ~30с повторяется `[Макс] health[err]: rej|Failed to register a ServiceWorker for scope`. Выглядит как «сбой связи», сбивает с толку при чтении журнала (легко погнаться за фантомным багом).

**Корень (по коду, НЕ поломка)**: (1) мы САМИ намеренно глушим ServiceWorker у всех веб-мессенджеров, кроме Ozon ([sessionSetup.js:77-88](../../main/utils/sessionSetup.js#L77-L88): `clearStorageData({serviceworkers})` + убийца по `running-status-changed`; см. также запись v1.2.22 выше «SW мёртв, потому что мы сами его глушим»). → у сайта `navigator.serviceWorker.register()` гарантированно отклоняется (факт ур.1 MDN `ServiceWorkerContainer.register()` → rejected Promise → `unhandledrejection`). (2) «ловец ошибок страницы» `attachRuntimeErrorCatcher` ([webviewDiagnostics.js:113-135](../../src/utils/webviewDiagnostics.js)) складывал ЛЮБУЮ ошибку/reject в `window.__ccLastErr`, а health-probe ([useWebViewLifecycle.js:19](../../src/hooks/useWebViewLifecycle.js#L19), раз в 30с) печатал её как `health[err]`.

**ВАЖНО (проверено)**: строку `health[err]` НИКТО не читает для статуса — в `consoleMessageHandler.js` нет потребителя `health[`. Значит цветная точка-статус мессенджера от этого НЕ краснеет; это ТОЛЬКО шум в журнале, а не индикация сбоя.

**Фикс (v1.2.332)**: в `attachRuntimeErrorCatcher` добавлен узкий фильтр `_ccSkip` — ошибки/reject с текстом `ServiceWorker` НЕ пишутся в `__ccLastErr`/журнал (это наш же самоинициированный отказ). Все остальные ошибки страницы ловятся как раньше. Откат: `git checkout src/utils/webviewDiagnostics.js`.

**Урок**: если приложение САМО что-то отключает у сайта (SW, уведомления, storage) — ожидаемый отказ этого нельзя тащить в журнал как «ошибку». Иначе диагностический шум маскирует настоящие ошибки. Фильтровать самоинициированные отказы у источника. **NB (v1.2.417)**: этот «безобидный отказ» для МАКС оказался НЕ безобидным — session-сторож убивал его SW по кругу → 429-шторм → чёрный экран. См. `mistakes/webview-navigation-ui.md` «SW-сторож убивает SW МАКС».

---

### 🟢 Тест логотипа-картинки проверяет только НАЧАЛО адреса, а не что картинка декодируется (v1.2.333, 2026-09-01)

**Грабля**: при добавлении логотипа мессенджера (data-URI в `messengerLogos.js`, напр. Ozon v1.2.333) юнит-тест `MessengerIcon.vitest.jsx` проверяет лишь, что `<img src>` начинается с `data:image/png;base64,`. jsdom (тестовый «пустой браузер») картинки НЕ декодирует → **обрезанный/битый base64 пройдёт тест зелёным**, а в приложении `<img onError>` тихо свалится на эмодзи-заглушку → фича «не работает», но тесты зелёные.

**Как надёжно (сделано в ревью v1.2.333)**: после вставки data-URI логотипа СВЕРИТЬ побайтно с исходным файлом — декодировать встроенный base64 и сравнить длину/байты с исходным PNG (проверить PNG-сигнатуру `89504E47` и наличие конца `IEND`). Пример проверки: `node -e '…Buffer.from(b64,"base64").equals(fs.readFileSync("PNG/X.png"))'`.

**Урок**: тест на data-URI ресурс, где движок картинку не рисует, доказывает только формат строки, НЕ целостность. Целостность встроенного бинарника проверять сравнением с источником.

---

## 🔴 Значок приложения не вшивается в .exe: `signAndEditExecutable: false` (v1.2.458, 2026-09-11)

**Симптом**: у ярлыка на рабочем столе (и у самого `ЦентрЧатов.exe`) стандартный значок Electron
(«атом»), хотя ВНУТРИ приложения — в окне и в трее — знак правильный.

**Почему сбивает с толку**: значок окна и трея приложение рисует НА ХОДУ своим кодом
(`nativeImage.createFromBuffer(drawMark(...))`). Ярлык так не умеет — он берёт картинку из файла
программы. Поэтому «внутри новый, на ярлыке старый» — это ОДНА беда, а не две.

**Корень**: в `package.json` → `build.win` стояло `"signAndEditExecutable": false`. Эта настройка
отключает не только подпись, но и ПРАВКУ РЕСУРСОВ файла — то есть вшивание значка и описания.
electron-builder писал об этом в журнале каждой сборки:
...
**Лечение**: `"signExecutable": false` — пропускается только подпись. Проверено фактом: значок,
извлечённый из собранного `.exe`, стал ОТЛИЧАТЬСЯ от значка чистого `electron.exe`, а описание файла
показало `ProductName: ЦентрЧатов`, `FileVersion: 1.2.458` (раньше было от Electron).

**Урок 1**: читать предупреждения сборщика — он сам назвал и причину, и лекарство.
**Урок 2**: 🟡 ДАТА ФАЙЛА — НЕ доказательство устаревания. Я сначала решил, что виноват ещё и
устаревший `build/icon.ico` (его дата была раньше правок рисовалки знака). Перегенерировал
(`node scripts/make-app-icon.cjs`) — файлы вышли ПОБАЙТНО ТЕМИ ЖЕ. Версия опровергнута; доказательство
устаревания — перегенерация и сравнение, а не время изменения.
**Урок 3**: 🟢 Windows КЭШИРУЕТ значки ярлыков. После правильной установки старый значок может
остаться — удалить ярлык и создать заново. Иначе легко решить, что фикс не помог.

**Страж**: `src/__tests__/projectHealth.test.cjs` — требует `signExecutable: false`, запрещает возврат
`signAndEditExecutable`, проверяет наличие `build/icon.ico`. Решение целиком — [[decisions]] ADR-057.
