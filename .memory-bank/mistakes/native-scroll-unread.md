...
## 🟡 Сильно-непрочитанный КАНАЛ «не запоминает» прокрутку вниз — перезагрузка ОКНА вокруг read-line (2026-08-03, расследование, фикс НЕ делался)

**Симптом.** Юзер 5 раз открывал канал (напр. «NN», 2.2М подписчиков, много непрочитанных), листал вниз — при возврате лента опять у того же старого сообщения (линия прочтения). НЕ помогли правки v1.2.188 (якорь-первым + churn guard) и v1.2.189 (компенсация разделителя).

**Почему прошлые правки не помогли (важно).** v1.2.188/189 работают на УРОВНЕ ЯКОРЯ — «к какому сообщению вернуться». Здесь проблема НИЖЕ: нужного сообщения нет в перезагруженном окне. Якорь не спасёт, если окна под ним нет.

**Корень (лог + код, 🟢 ПОДТВЕРЖДЁН).** Обработчик входящих `tg:messages` при обычной загрузке (не append/appendNewer) делает `next = messages` — [nativeStoreIpc.js:244](../../src/native/store/nativeStoreIpc.js) — то есть ЗАМЕНЯЕТ уже выросший список свежим окном. Лог прямо показал: `store-tg-messages append=false appendNewer=false incoming=100 existing=198 → action=replaced` (было 198, стало 100). На каждом открытии `loadMessages` тянет ~100 вокруг СТАРОГО `readInboxMaxId` (`store-load-messages aroundId=16822304768 override=false`) → всё, что юзер долистал, выбрасывается (высота ленты 94251→45760). Догрузка вниз `appendNewer` наращивать УМЕЕТ (в [nativeStoreIpc.js:226-242](../../src/native/store/nativeStoreIpc.js) `next=[...existing,...newNewer]`, без капа), но толку нет — следующее открытие снова заменяет список. `readInboxMaxId` остаётся старым, т.к. в канале сотни непрочитанных и они не помечаются прочитанными скопом → окно всегда вокруг одной старой точки → «застряло».

**Это НЕ регрессия v1.2.188-190.** Замена-на-открытии — давняя логика (эпоха v0.95.x), мои правки её не трогали. Правки-якоря НЕ лечат: якорь указывает на сообщение, которого нет в заменённом окне.

**Фикс (НЕ делал — опасная зона).** На уровне ОКНА (store), не якоря: при повторном открытии уже-виденного чата не заменять выросший список свежим окном (либо мерджить, либо не перезагружать если кэш свежий). ⚠️ Затрагивает ВСЕ чаты + расчёт первого непрочитанного + подгрузку вверх/вниз — менять с большой осторожностью и обязательной визуальной проверкой на разных чатах (личка/группа/форум-тема/канал).

**Как не спутать.** Признак ЭТОГО бага (в отличие от «сползания на 46px», ниже): высота ленты РЕЗКО меняется на открытии (десятки тысяч px), а `load-newer` идёт по кругу с одним `afterId`. Признак «сползания»: высота ±46px, `scrollTop` дрейфует помалу.

---

## 🔴 Непрочитанный канал сползает вниз ~46px/визит — разделитель «Новые сообщения», НЕ закреп (2026-08-03, v1.2.189)

**Симптом.** Конкретный КАНАЛ с большим числом непрочитанных при переключении сползает вниз на ~46px за заход, копится. Юзер подозревал плашку «Закреплённое».

**Ложный след (НЕ он).** Плашка закрепа — `position:absolute` накладка ([PinnedMessageBar.jsx](../../src/native/components/PinnedMessageBar.jsx), v0.95.5), высоту ленты НЕ меняет. Проверяется в логе: если бы толкал закреп, менялась бы высота ленты при его появлении — но менял её другой элемент.

**Настоящий корень (лог: `scrollHeight` 47877↔47923 = +46px синхронно с `loading`; scrollTop −46/визит).** Разделитель **«Новые сообщения»** ([VirtualMessageList.jsx](../../src/native/components/VirtualMessageList.jsx) `type==='unread'`) вставляется ВЫШЕ первого непрочитанного ([messageGrouping.js:44-47](../../src/native/utils/messageGrouping.js)). `firstUnreadId` приходит `null→value` АСИНХРОННО ([InboxMode.jsx:487](../../src/native/modes/InboxMode.jsx)) уже ПОСЛЕ restore → разделитель (~46px) толкает якорь (= первое непрочитанное) вниз; сдвиг попадает в автосейв и накапливается. `overflow-anchor:none` (намеренно, [VirtualMessageList.jsx:237](../../src/native/components/VirtualMessageList.jsx)) → браузер позицию сам не держит, компенсируем вручную.

**Как не повторить.** Любой элемент, который вставляется ВЫШЕ якоря асинхронно (разделитель непрочитанного, дата, будущие баннеры), сдвигает сохранённую позицию при `overflow-anchor:none`. Приём: захватить верхнее видимое сообщение ДО вставки и вернуть его на пиксель ПОСЛЕ (DOMRect re-pin, как load-older). Диагностика: сравнить `scrollHeight` при `restore-applied` и через ~100мс — скачок = элемент вставился.

**Фикс — v1.2.189:** отдельный `unreadDividerAnchorRef` + `useLayoutEffect` на `firstUnreadId` (re-pin), не трогая `prependAnchorRef` (load-older) и `overflow-anchor`. Событие лога `unread-divider-compensate`.

---

## 🔴 Восстановление позиции при переходе между чатами «плывёт» — вернулось без виртуализации (2026-08-03, v1.2.185–187)

**Симптом.** Юзер: «перемещаюсь между чатами — позиция в том, что был до этого, меняется». Оставил чат на месте X, зашёл в другой, вернулся — X сместился. Быстрое A↔B усиливает. Это **возврат старой саги** (была закрыта 9 июня 2026) уже в НЕ-виртуальном DOM.

**Три корня (подтверждены логом 2026-08-03):**
1. Защита `isRestoringRef` держится только 500мс ([useInitialScroll.js:53](../../src/native/hooks/useInitialScroll.js)), а на открытии чата идёт `load-newer` (100 сообщений) + перезагрузка окна `aroundId=readInboxMaxId` — доезжают ПОЗЖЕ 500мс → сохранение уже не заблокировано, пишет «дышащую» позицию (`scroll-save REST=false`).
2. Флаг `atBottom=true` главнее якоря и «залипает»: `if (saved?.atBottom) el.scrollTop = scrollHeight` выбрасывает точный якорь-середину. `atBottom` пишется по порогу `<80px` даже при осмысленном якоре выше низа (лог: `anchorMsgId=… screenTop=-444 atBottom=true` целую минуту неподвижно).
3. Окно грузится вокруг линии прочтения, а не вокруг якоря → якоря может не быть в DOM → откат в конец (`anchor-missing`); между визитами окно разное → одна метка даёт `mode=bottom` и `mode=anchor`.

**Как не повторить.** Не наслаивать формат/троттлинг поверх недобитого «замкнутого круга сохранения» (ровно это и сделали в v1.2.185–187 — три попытки, жалоба осталась). Сначала добить guard (пункт 1) и `atBottom`-override (пункт 2).

**Фикс — v1.2.188** (ждёт визуальной проверки): (②) при восстановлении **якорь ПЕРВЫМ**, `atBottom` — только запасной путь ([useInitialScroll.js](../../src/native/hooks/useInitialScroll.js)); (①) **churn guard** — пока `loadingNewerRef`/`loadingOlderRef` в работе, оба пути сохранения ([useInboxScroll.js](../../src/native/hooks/useInboxScroll.js), [useScrollPositionAutosave.js](../../src/native/hooks/useScrollPositionAutosave.js)) позицию не пишут. Полная история — в [[native-scroll-restore-saga-CLOSED]] ([archive/native-scroll-restore-saga-CLOSED.md](../archive/native-scroll-restore-saga-CLOSED.md), раздел «🔁 2026-08-03»).

---

## 🟡 Перетаскивание порядка строк ВНУТРИ react-window — тупик (2026-07-30, v1.2.156–162)

**Симптом.** Задача — дать перетаскивание порядка закреплённых чатов «как в Telegram». Ушло 5 итераций (v1.2.156–160) и крупный откат (v1.2.161), прежде чем стало ясно, что подход неверный в корне.

**Грабля техническая.** Список чатов — `react-window@2.2.7`: строки ставятся `transform: translateY(индекс×высота)` и ключуются по ИНДЕКСУ (`key=index`, подтверждено в `dist/react-window.cjs`). Поэтому при смене порядка DOM-узел остаётся на своём слоте, меняется лишь содержимое — плавно «разъезжать» соседей (как в Telegram) внутри react-window НЕЛЬЗЯ. Вынести закреплённые в отдельный не-виртуальный список — можно перетаскивать, НО тот блок рендерится вне прокрутки основного списка → «приклеен», не листается вместе со списком (это и была жалоба). Итог: «drag-перестановка» и «строки в общем виртуальном списке» несовместимы без замены движка. Решение зафиксировано в [[decisions]] ADR-025.

**Грабля процессная (важнее).** Пользователь просил «как в Telegram», а я 4 раза чинил визуал (призрак→карточка→слот→вынос), не уточнив ГЛАВНОЕ требование: должны ли закреплённые листаться вместе со списком. Когда наконец уточнил — оказалось, что вся ветка выноса не нужна. **Урок:** перед КРУПНОЙ переделкой (замена рендера списка, новый компонент) — сначала одним вопросом зафиксировать ключевое требование, особенно если UI нельзя проверить без запуска приложения. Дешёвый вопрос экономит 5 итераций.

**Как не повторить.** Не пытаться делать drag-and-drop порядка внутри `react-window` списка. Если снова попросят перетаскивание закреплённых — сослаться на ADR-025 и сначала согласовать: отдельный блок (не листается) ИЛИ замена виртуального списка.
