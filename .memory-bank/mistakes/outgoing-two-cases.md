...
## 🔴 Тот же разовый статус НЕЛЬЗЯ затирать другим частым событием (v1.2.230)

**Симптом**: галочка в СПИСКЕ чатов показывает зелёную двойную (прочитано) после загрузки, но через секунду после старта гаснет до одной серой (отправлено). В ОКНЕ чата при этом всё верно (зелёная).

**Причина (по коду)**: список и окно используют РАЗНЫЕ поля — окно `message.isRead`, список `chat.lastMessageRead`. Живой обработчик `tg:chat-last-message` ([nativeStoreLastMsgIpc.js](../../src/native/store/nativeStoreLastMsgIpc.js)) жёстко ставил `lastMessageRead: false` на любое событие. А TDLib на старте шлёт `updateChatLastMessage` по каждому чату заново (сотни за <2с) → верный статус, посчитанный `mapChat` при загрузке, затирался в `false`. Сигнал `updateChatReadOutbox` для УЖЕ прочитанного не повторяется → назад не поднять.

**Грабли**: недостаточно вычислить разовый статус при загрузке (урок выше). Есть ВТОРОЙ частый служебный поток (`updateChatLastMessage`), который эмитится массово на старте и на любое изменение порядка/превью — он НЕ должен «ронять» ранее посчитанный статус в дефолт. Особенно опасно, что окно (другое поле) остаётся верным → баг выглядит как «список врёт», хотя данные приходили правильные.

**Правильно (v1.2.230)**: событие `chat:last-message` несёт НАСТОЯЩИЙ статус (`lastMessageRead`/`lastMessageId`/`lastMessageSending`), посчитанный backend по `last_read_outbox_message_id` в `tdlibClient._patchChat` (в момент эмита это поле уже доступно) — ровно так же, как `mapChat` при загрузке. Стор применяет присланный статус, а не хардкод. Единая логика для загрузки и живого пути.

**Общее правило**: если поле дублируется в двух местах UI (окно ↔ список), они должны питаться из ОДНОГО расчёта. И частое служебное событие (last-message, reorder) обязано либо нести полный корректный статус, либо НЕ трогать поле вовсе — но не сбрасывать в дефолт.
