...
С v1.2.30 диагностика работает как ручная фоновая сессия в renderer:
- открытие большой модалки не запускает запись автоматически;
- `Включить запись` запускает `useDiagnosticsSession`, после чего он раз в 3 секунды вызывает `app:diagnostics-snapshot`, анализирует снимок и кладёт важные события в ограниченный ring-buffer;
- глубокая WebView-проверка всегда включена и не имеет отдельного переключателя;
- `Свернуть в фон` закрывает большую модалку, но оставляет маленькую панель и продолжает запись;
- `Развернуть` в маленькой панели открывает большую модалку без повторного старта записи;
- после `Стоп` маленькая панель показывает накопленный буфер и даёт кнопку `Запустить`, чтобы начать новую запись без перезапуска окна;
- `Закрыть` в маленькой панели полностью сбрасывает диагностическую сессию и скрывает панель;
- `Отключить и сохранить` останавливает запись и сохраняет `diagnosticsSession.events`, чтобы ИИ мог восстановить цепочку;
- `Очистить экран` чистит только UI/ring-buffer диагностики, но не `chatcenter.log` и не `ai-errors.log`.
- `SettingsPanel` получает `diagnosticsStatus` из `DiagnosticsSessionHost` и показывает рядом с кнопкой `🩺 Диагностика системы` состояние записи: выключена/включена/пауза + количество событий.
...
🕓 **ПРОШЛЫЙ ЖУРНАЛ — `chatcenter.prev.log` (v1.2.468–469)**. Основной `chatcenter.log` при
переполнении (предел 2 МБ) обрезается пополам. Раньше отрезанная половина ИСЧЕЗАЛА, и жалобу
вроде «такое бывает часто» подтвердить было нечем (реальный случай 2026-09-16: на жалобу про
уведомление Ozon в журнале нашлась ровно одна запись). Теперь она сохраняется в соседний файл,
а в окне «📒 Логи ЦентрЧатов» есть кнопка **«🕓 Прошлый журнал»** — с той же подсветкой и
фильтрами. Канал `app:read-prev-log` возвращает `{ exists, content }`; 🔴 путь берётся ТОЛЬКО
из кода (`getPrevLogFilePath` в `main/utils/logger.js`), извне не принимается. При показе
прошлого журнала автообновление ВЫКЛЮЧАЕТСЯ — иначе свежие строки затёрли бы показанное.
**При разборе жалоб «часто/повторяется» смотреть ОБА файла.**
