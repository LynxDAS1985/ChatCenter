...
**Ловушка 6 (v0.63.3)**: Тултип в ribbon — НЕ использовать `title` атрибут (некрасивый, с задержкой). Использовать кастомный `position: fixed` div с `pointer-events: none`. Показывать ТОЛЬКО если `scrollWidth > clientWidth` (текст реально обрезан). CSS-класс `.cc-tooltip` (не `.tooltip` — может конфликтовать).
