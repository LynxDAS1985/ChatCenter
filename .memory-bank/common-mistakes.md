...
### ❌ Бейдж/крестик вкладки перекрывает соседние элементы

**Симптом**: Бейдж непрочитанных (например "47") или крестик закрытия вкладки налезает на логотип "ЦентрЧатов" или соседнюю вкладку.

**Причина**: Бейдж и крестик позиционированы `absolute top-1 right-1` внутри `relative` кнопки вкладки. При маленьком `padding` (px-3 = 12px) absolute-элементы визуально выходят за правый край вкладки и перекрывают соседей.

**Решение**: Увеличить правый отступ вкладки и добавить минимальную ширину:
```jsx
// БЫЛО:  px-3 — бейдж вылезает
// СТАЛО: pl-3 pr-6 min-w-[90px] — место для бейджа справа
className="relative flex items-center gap-2 h-[40px] pl-3 pr-6 min-w-[90px] ..."
```
