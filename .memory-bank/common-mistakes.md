...
**Ловушка 37 (v0.73.8)**: JS-блокировка Service Worker в WebView (`navigator.serviceWorker.register` override + `getRegistrations().unregister()`) НЕ РАБОТАЕТ для закешированных SW. Закешированный SW в partition storage (`persist:telegram`) активируется Chromium ДО выполнения нашего JS-кода (preload script tag injection / executeJavaScript). SW успевает вызвать `navigator.setAppBadge(N)` → Chromium C++ ставит overlay icon → наш overlay перебивается. **ПРАВИЛО**: Блокировать SW на уровне **Electron session** через `ses.clearStorageData({ storages: ['serviceworkers', 'cachestorage'] })` в `setupSession()`. Это удаляет закешированные SW ДО загрузки страницы. JS-блокировку `register()` оставить как страховку от НОВЫХ регистраций. Мониторить через `ses.serviceWorkers.on('running-status-changed')` — если SW всё-таки стартовал, повторная очистка.
