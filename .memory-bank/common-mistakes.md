...
**Ловушка 62 (v0.86.2)**: WhatsApp sidebar watcher давал больше фантомов чем пользы. **Симптом**: ribbon показывал `ic-expand-more`, `status-dblcheck`, `Фото`, `печатает...` вместо реальных сообщений. **Корень проблемы**: ошибочное предположение что WhatsApp Web Business не шлёт `new Notification()` при активной вкладке. **Реальность из логов 06:29:21**: WhatsApp ШЛЁТ Notification API даже при активной вкладке — "Выя", "Ауыч" приходили через `__CC_NOTIF__` нормально. Sidebar watcher был не нужен. **Sidebar watcher собирал текст из `span[dir]` и `span[class]` в row sidebar — попадал в текст SVG-иконок (`ic-expand-more`, `status-dblcheck`) и в превью старых сообщений (`Фото`).** **Фикс**: полностью убран sidebar watcher. Уведомления приходят только через Notification API hook (как Telegram). **ПРАВИЛО**: НЕ строить решения на ошибочных предположениях о поведении мессенджера. ВСЕГДА проверять логи реальных сообщений ДО написания workaround. Sidebar/MutationObserver — крайний случай, не дефолт. **Урок**: 5 итераций решений (visibility, hash, mousedown, sidebar) основаны на неточном анализе логов. Если sender приходит в `__CC_NOTIF__` — значит Notification API работает.
