...
**Ловушка 30 (v0.72.1)**: Контекстное меню моргает при ПКМ когда уже открыто. Причина: `mousedown` (глобальный handler) срабатывает ДО `contextmenu`. `mousedown` закрывает старое меню с fade-out анимацией (150ms), `ctxMenuEl = null`. Потом `contextmenu` → `showCtxMenu()` → `hideCtxMenu(true)` ничего не делает (ctxMenuEl=null), а старый DOM-элемент с `.closing` классом ещё в DOM → новое меню создаётся параллельно → пользователь видит fade-out + fade-in = моргание. **ПРАВИЛО**: В `mousedown` handler: `if (e.button === 2 && e.target.closest('.dock-tab')) return` — не закрывать при ПКМ на табе, `contextmenu` handler сам переоткроет. В `showCtxMenu()`: `document.querySelectorAll('.ctx-menu').forEach(el => el.remove())` — удалить ВСЕ остатки (включая closing-элементы).
