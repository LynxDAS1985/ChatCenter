...
| `common-mistakes.md` | **Индекс** ловушек (детали в `mistakes/`, в т.ч. `mistakes/app-layout.md` — раскладка нашего экрана, создан 2026-09-11) | При ошибках/отладке |
...
| `notif-window-input-loss-case.md` | 🕵️ ДЕЛО: окно уведомлений перестаёт принимать мышь — все кнопки мертвы (3 случая: 19.08, 02.09, 19.09.2026). Факты, опровергнутое, 5 версий причин, дока Electron, что ловим в журнале и как читать, план, журнал наблюдений | При жалобе «кнопки уведомления не нажимаются / не закрывается»; перед правками `setAlwaysOnTop`/«пружины»/`focusable` окна уведомлений |
| `notifications-debug.md` | 🔔 Отладка уведомлений: схема «кто шлёт» по каждому мессенджеру (у веб — до трёх независимых источников), таблица меток в журнале, известные беды по мессенджерам, журнал разобранных случаев | При ЛЮБОЙ жалобе на уведомления: дубли, пустой текст, не приходит, мусор в тексте |
...
| `mistakes/notifications-ribbon.md` | Кастомные уведомления, ribbon BrowserWindow, позиционирование дока/подсказки, вид карточки | ~74 КБ |
...
- `mistakes/webview-injection.md` 130 КБ → `webview-injection.md` 9 КБ (ядро) + `webview-stack-grouping.md` 125 КБ (v0.87.68)
