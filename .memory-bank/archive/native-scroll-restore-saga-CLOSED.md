> 🟡 **STATUS: ФИКС РЕАЛИЗОВАН v1.2.188 (3 августа 2026), ЖДЁТ ВИЗУАЛЬНОЙ ПРОВЕРКИ** — жалоба вернулась
> после удаления виртуализации (v0.94.0). Юзер: «позиция в прошлом чате меняется при переходе между чатами».
> Разбор + фикс — в разделе «🔁 2026-08-03» СРАЗУ НИЖЕ (2 корня: ② якорь-первым, ① churn guard). Историю
> ниже (v0.91.x, эпоха react-window) читать как **контекст**: движок сменился, но корневой механизм
> «замкнутый круг сохранения» тот же.
>
> ⬇️ Ниже (под разделом 2026-08-03) — архивная закрытая версия от 9 июня 2026.
>
> ---
>
> # 🔁 2026-08-03 (v1.2.185–187): проблема вернулась без виртуализации
>
> **Жалоба юзера (дословно).** «Я перемещаюсь между чатами, но если я листаю другие положения — в том,
> что был до этого, меняется». То есть: оставил чат на месте X, зашёл в другой, вернулся — X сместился.
> Быстрое переключение A↔B усиливает.
>
> ## Что делали (3 попытки) и почему не вышло
>
> | Версия | Что сделали | Результат |
> |---|---|---|
> | **v1.2.185** | Формат позиции `{fromBottom}` — расстояние от НИЗА | 🔴 Юзер отверг: «от низа тоже не то, надо ровно то место, где остановился» |
> | **v1.2.186** | Якорь по сообщению + смещение (`{anchorMsgId, screenTop, atBottom}`), паттерн Telegram Web K ScrollSaver. `computeScrollAnchor` / `placeAnchor` в [scrollPositionsCache.js](../../src/native/utils/scrollPositionsCache.js) | 🟡 Принят как подход, НО жалоба осталась — позиция всё равно «плывёт» |
> | **v1.2.187** | Троттлинг вычисления якоря 250мс (по ревью — производительность) | 🟡 Не про эту жалобу (только скорость) |
>
> ## Корень (найден 3 августа 2026 по свежему `chatcenter.log`) — ТРИ дефекта
>
> Механизм тот же «замкнутый круг сохранения», что и в v0.91.19, но теперь его кормят догрузка окна и
> флаг `atBottom`. Три слабых места (все — уровень 2, код проекта):
>
> **Дефект 1 — защита `isRestoringRef` живёт только 500мс, а догрузка приходит позже.**
> [useInitialScroll.js:53](../../src/native/hooks/useInitialScroll.js) сбрасывает флаг через `setTimeout(..., 500)`.
> Но при открытии чата идёт `store-load-messages aroundId=readInboxMaxId` + сразу `load-newer` (100 сообщений),
> которые доезжают ПОЗЖЕ 500мс. Список «дышит» (высота ...773 скакала 32364→32410→15503), а сохранение уже
> НЕ заблокировано. Лог-доказательство: `scroll-save ...773 REST=false atBottom=false/true` через 1–2с после
> переключения.
>
> **Дефект 2 — флаг `atBottom=true` ГЛАВНЕЕ якоря, и он «залипает».**
> При восстановлении [useInitialScroll.js:90](../../src/native/hooks/useInitialScroll.js): `if (saved?.atBottom) el.scrollTop = el.scrollHeight`
> — то есть при `atBottom=true` точный якорь-середина ВЫБРАСЫВАЕТСЯ, прыжок в конец. А `atBottom` пишется
> механически по порогу `<80px` даже когда есть осмысленный якорь-середина. Лог-доказательство (противоречие):
> ```
> 16:19:02–16:19:17  autosave-save ...057 anchorMsgId=9357492224 screenTop=-444 atBottom=true
> ```
> Юзер ЦЕЛУЮ МИНУТУ сидит неподвижно, якорь на середине (−444), но `atBottom=true` пишется непрерывно →
> следующий вход прыгнет в конец. Дальше самоподдержка: раз «внизу» → вход в конец → `load-newer` → снова
> «внизу» → снова сохранилось «внизу».
>
> **Дефект 3 — окно сообщений перезагружается вокруг линии прочтения, а не вокруг якоря.**
> `store-load-messages ... aroundId=readInboxMaxId` на каждом открытии тянет свежее окно вокруг «непрочитанного»,
> а не вокруг сохранённого якоря. Если якорь-сообщение не попало в это окно → `placeAnchor` не находит его →
> откат в конец (`mode=anchor-missing`). Между визитами окно разное → та же метка даёт разный результат
> (`mode=bottom` в 16:15:45, `mode=anchor` в 16:15:54 — тот же чат).
>
> ## Фикс — РЕАЛИЗОВАН в v1.2.188 (ждёт визуальной проверки)
>
> 1. ✅ **Корень ② (главный) — якорь ПЕРВЫМ при восстановлении.** [useInitialScroll.js](../../src/native/hooks/useInitialScroll.js)
>    ветка 2: сперва `placeAnchor`, и только если якоря нет в окне — откат в конец. `atBottom=true` больше
>    не выбрасывает точную середину. Тест-ловушка в [useInitialScroll.vitest.jsx](../../src/native/hooks/useInitialScroll.vitest.jsx).
> 2. ✅ **Корень ① — churn guard.** Пока `loadingNewerRef`/`loadingOlderRef` в работе, оба пути сохранения
>    ([useInboxScroll.js](../../src/native/hooks/useInboxScroll.js), [useScrollPositionAutosave.js](../../src/native/hooks/useScrollPositionAutosave.js))
>    позицию не пишут (само-таймится по завершению догрузки). Вызов автосейва в InboxMode перенесён ниже
>    объявления флагов.
> 3. ⏸️ (не делали) Подгрузка истории ВОКРУГ якоря, если его нет в окне — оставлено как запас: сейчас
>    откат в конец, как раньше. Включить, если после проверки якорь иногда «не находится».
>
> **Осознанный размен:** возврат в чат, где был у самого низа, теперь ставит на сохранённый якорь (то же
> место), а не догоняет новые сообщения, пришедшие пока тебя не было — это и есть «ровно то место, где
> остановился». Догон новых при ОТКРЫТОМ чате — отдельная логика (physicallyAtBottom), не затронута.
>
> ## Продолжение — v1.2.189: непрочитанный КАНАЛ сползал ~46px/визит (разделитель «Новые сообщения»)
>
> После v1.2.188 юзер заметил, что конкретный канал с большим непрочитанным всё равно сползает вниз, и
> подозревал плашку «Закреплённое». Разбор: закреп НЕ виноват (`position:absolute` накладка). Виноват
> разделитель **«Новые сообщения»** — он вставляется ВЫШЕ первого непрочитанного (= якорь канала)
> АСИНХРОННО (`firstUnreadId` null→value) уже ПОСЛЕ restore, толкая якорь вниз на ~46px; сдвиг копился.
> Лог: `scrollHeight` 47877↔47923 (+46). Факт уровня 1 (MDN `overflow-anchor:none`): вставка выше вьюпорта
> сдвигает видимое, браузер сам не держит. Фикс: DOMRect re-pin (как load-older) в ОТДЕЛЬНОМ
> `unreadDividerAnchorRef` + `useLayoutEffect` на `firstUnreadId`, `overflow-anchor` не трогали. Подробности —
> [[native-scroll-unread]] и features.md v1.2.189.
>
> ## Урок (тот же, что в 2026 — не забыт, а нарушён средой)
>
> Корень подтверждён логами ДО правки (правильно). Но три попытки v1.2.185–187 наслаивали формат/троттлинг
> поверх недобитого «замкнутого круга» — ровно антипаттерн из «Главного урока» ниже. `atBottom`-override и
> короткий 500мс-guard остались от эпохи react-window и в новом (не-виртуальном) DOM работают против нас.
>
> ---
>
> ## 📦 Ниже — архивная версия (закрыта 9 июня 2026)
>
> > ✅ **STATUS: CLOSED 9 июня 2026 по решению юзера** (исторический контекст ниже; движок с тех пор сменён —
> > react-window удалён в v0.94.0)
>
> ---
