# Архив: хронология расследования долгой загрузки при старте

Вынесено из [`../startup-load-investigation.md`](../startup-load-investigation.md) 2026-09-16 (v1.2.468):
активный файл дорос до предела 100 КБ, и `npm run check-memory` предупреждал «приближается».

Здесь — **подробная хронология расследования** (протоколы запусков, разборы логов по датам).
Постановка задачи и ИТОГ остались в активном файле — начинать надо с него.

Темы: разборы логов 6 мая 2026 по часам, что делалось в v0.87.125–v0.87.133, промежуточные версии причины.
