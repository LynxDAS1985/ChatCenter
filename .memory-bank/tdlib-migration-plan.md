...
| **2.3** TDLib authorization flow | (текущий) | `backends/tdlibAuth.js` 313 строк: `buildTdlibParameters()` + `TdlibAuthFlow` (state machine waitTdlibParameters → waitPhoneNumber → waitCode → waitPassword → ready). Внешний API: `startLogin(phone)`, `submitCode(code)`, `submitPassword(password)`, `cancelLogin()` — совместим с GramJS `tg:login-*`. Resolvers ставятся синхронно ПЕРЕД invoke (race-free). 19 vitest тестов с mock TDLib client. |
