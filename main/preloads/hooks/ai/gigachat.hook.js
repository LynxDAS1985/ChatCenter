// v1.2.0 (Этап 6 AI Bridge): hook для giga.chat.
// Сбер использует свой stack (Angular?) — селекторы НЕИЗВЕСТНЫ точно. Юзер должен
// настроить через UI (Этап 8) если defaults не работают.

;(function () {
  const PROVIDER = 'gigachat'
  ...
  let SELECTORS = {
    input: 'textarea, [contenteditable="true"], input[type="text"][placeholder*="вопрос"]',
    submitButton: 'button[type="submit"], button[aria-label*="Отправ"], button[class*="send"]',
    lastAssistantMessage: '[class*="message"][class*="assistant"]:last-of-type, [class*="bot"]:last-of-type, [class*="answer"]:last-of-type',
    streamingIndicator: '[class*="typing"], [class*="loading"], [class*="progress"]',
  }
      ...
      if (now - startedAt > MAX_WAIT_FOR_ANSWER_MS) {
        try {
          window.__ccAiBridge.error(questionId, 'streaming_timeout',
            'Ответ ГигаЧат не появился за ' + MAX_WAIT_FOR_ANSWER_MS + 'мс')
        } catch (_) {}
        teardown(); return
      }
    ...
    if (!input) {
      try {
        window.__ccAiBridge.error(questionId, 'input_not_found',
          'Поле ввода ГигаЧат не найдено. Откройте настройки и укажите свой селектор для input.')
      } catch (_) {}
      return
    }
    ...
    setTimeout(function () {
      ...
      if (btn && !btn.disabled) {
        ...
        try {
          ...
          try { window.__ccAiBridge.error(questionId, 'submit_not_found', 'Кнопка отправки ГигаЧат не найдена — настройте селектор в настройках') } catch (_) {}
        }
      }
    }, INJECT_DELAY)
  }
