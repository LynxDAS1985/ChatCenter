...
(function() {
  ...
  // v1.2.482: что считается СТАТУСОМ, а не сообщением (проверка целиком, регистр не важен).
  var _waStatusOnly = /^(печатает|печатают|набирает( текст)?|записывает( аудио| видео| голосовое)?|онлайн|в сети|был[аи]? недавно|typing|recording( audio| video)?|online|last seen recently)$/i;
    ...
    _sidebarObserver = new MutationObserver(function() {
      ...
      try {
        ...
        for (var i = 0; i < Math.min(rows.length, 15); i++) {
          ...
          if (!lastMsg) continue;
          // v1.2.482: строка чата на миг показывает СТАТУС («печатает…», «в сети») вместо текста сообщения.
          // Принять его за новое сообщение нельзя ДВАЖДЫ: во-первых, это ложная карточка; во-вторых, статус
          // попадёт в память «последний текст», и когда вернётся настоящий текст — он уедет как «новое» =
          // ВТОРАЯ карточка на одно сообщение. Тот же корень чинили у ВК (ADR-072) и Ozon (v1.2.437).
          // Маски проверяют строку ЦЕЛИКОМ (^…$) — живое сообщение со словом «печатает» не пострадает.
          if (_waStatusOnly.test(lastMsg.replace(/[.…\s]+$/, ''))) continue;
