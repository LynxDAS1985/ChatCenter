...
(function() {
  ...
  function _maxScanList(root, emit) { _maxTrackTitleUnread(); var search = _maxSearchState();
    ...
    var rows = root.querySelectorAll('[class*="wrapper--withActions"], [role="listitem"], [role="presentation"]'); for (var i = 0; i < rows.length && i < 60; i++) {
      ...
      console.log('__CC_DIAG__max-sidebar-decision: ' + JSON.stringify({ action:action, sender:info.sender, body:info.body, prevBody:prevBody||'', unread:info.unread, prevUnread:prevUnread, firstSeen:firstSeen, bodyChanged:bodyChanged, unreadIncreased:unreadIncreased, stableUnreadBodyChanged:stableUnreadBodyChanged, freshTitleMs:freshTitleMs, emit:!!emit, search:search, title:document.title||'', url:location.href||'', ts:Date.now() })); if (stableUnreadBodyChanged) { try { var _tb=(document.querySelector('.topbar')||{}).textContent||'', _pm=[]; Array.prototype.forEach.call(rows[i].querySelectorAll('svg,use,[class*="check" i],[class*="read" i],[class*="status" i],[class*="tick" i],[class*="sent" i],[aria-label]'), function(e){ var cl=(typeof e.className==='string'?e.className:(e.className&&e.className.baseVal)||''), al=(e.getAttribute&&e.getAttribute('aria-label'))||'', hr=(e.getAttribute&&(e.getAttribute('href')||e.getAttribute('xlink:href')))||''; if(cl||al||hr)_pm.push((e.tagName||'')+'|'+String(cl).slice(0,26)+'|'+al.slice(0,16)+'|'+hr.slice(0,16)); }); console.log('__CC_DIAG__max-own-probe: sender="'+info.sender+'" tbMatch='+(_tb.indexOf(info.sender)>=0)+' body="'+(info.body||'').slice(0,22)+'" marks='+JSON.stringify(_pm.slice(0,12))); } catch(e){} }
