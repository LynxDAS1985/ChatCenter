      ...
      try {
        ...
        for (var d = 0; d < rows.length; d++) {
          ...
          for (var t = 0; t < tds.length; t++) {
            ...
            if (/^(Получен|Отправлен|Отменён|Отменен|Доставлен|Возврат|Ожидает|Опубликован)$/.test(ct)) continue; // v1.2.389: статус — ТОЛЬКО ЦЕЛИКОМ (якорь $, как у бейджа выше). Иначе отзыв «Получено спасибо» примут за статус «Получен» и он пропадёт (тело станет «Оценка без текста»).
