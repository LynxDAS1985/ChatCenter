...
export default function SystemDiagnosticsModal({ onClose, onMinimize, runtimeContext, onRunDeepCheck, diagnosticsSession, diagnosticsActions }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (session.deepWebview && runDeepRef.current) await Promise.resolve(runDeepRef.current())
  ...
  return (
    <div style={css.overlay} onClick={onClose}><div style={css.modal} onClick={e => e.stopPropagation()}>
      ...
      <div style={css.body}>
        <div style={{ ...css.card, display: 'grid', gap: 10 }}><div style={{ fontWeight: 700 }}>Фоновая диагностическая сессия</div><div style={css.row}>{!session.active && <ActionButton kind="primary" onClick={diagnosticsActions?.start}>Включить запись</ActionButton>}{session.active && !session.paused && <ActionButton onClick={diagnosticsActions?.pause}>Пауза</ActionButton>}{session.active && session.paused && <ActionButton kind="primary" onClick={diagnosticsActions?.resume}>Продолжить</ActionButton>}{session.active && <ActionButton kind="danger" onClick={diagnosticsActions?.stop}>Отключить и сохранить</ActionButton>}<ActionButton active={!!session.deepWebview} onClick={diagnosticsActions?.toggleDeep}>Глубокая WebView: {session.deepWebview ? 'вкл' : 'выкл'}</ActionButton>{(session.active || sessionEvents.length > 0) && <ActionButton onClick={onMinimize || onClose}>Свернуть в фон</ActionButton>}</div><div style={css.muted}>Статус: {sessionStatus}. Буфер: {sessionEvents.length} событий. Окно само запись не запускает: нажмите "Включить запись", когда нужно поймать проблему.</div><div style={{ ...css.muted, color: session.lastError ? '#f87171' : '#93c5fd' }}>{session.lastError || session.lastSavedPath || 'При сворачивании запись продолжится в маленькой панели. Кнопка "Закрыть" на маленькой панели выключает диагностику полностью.'}</div></div>
        <div style={{ ...css.card, display: 'grid', gap: 10 }}><div style={{ fontWeight: 700 }}>Отчёт для разбора</div><div style={css.row}><ActionButton onClick={saveForAi} disabled={!canUseReport} minWidth={132}>Сохранить для ИИ</ActionButton><ActionButton onClick={copyForAi} disabled={!canUseReport} minWidth={132}>Скопировать для ИИ</ActionButton><ActionButton kind="danger" onClick={clearScreen} minWidth={118}>Очистить экран</ActionButton></div><div style={css.muted}>Большая модалка показывает снимок и накопленную сессию. Непрерывная запись работает только после ручного включения.</div><div style={{ ...css.muted, color: '#93c5fd', minHeight: 18 }}>{message || 'Общий chatcenter.log и ai-errors.log эта панель не очищает.'}</div></div>
