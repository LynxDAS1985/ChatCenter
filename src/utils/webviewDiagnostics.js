...
export function runVkFullProbe(el, messengerId) {
  ...
  const script = `(function(){try{
    ...
    function outgoingEvidence(el){var c=cls(el),aria=attr(el,'aria-label'),data=attr(el,'data-out')||attr(el,'data-outgoing')||attr(el,'data-own'),cm=c.match(/out|own|self|sent|ConvoMessage--out|im-mess_out|message_out/i),dm=/^(1|true|yes)$/i.test(data),am=/you sent/i.test(aria)||/РІС‹ РѕС‚РїСЂР°РІРёР»Рё|РёСЃС…РѕРґСЏС‰/i.test(aria);return{result:!!(cm||dm||am),reason:cm?('class:'+cm[0]):dm?'data-outgoing':am?'aria-outgoing':'',className:c,dataOut:data,aria:aria,checks:{classBroadOutOwnSelfSent:!!cm,dataOutTrue:dm,ariaOutgoing:am}}}
