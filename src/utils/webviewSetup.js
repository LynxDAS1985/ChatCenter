...
export function createWebviewSetup(deps) {
  ...
  const setWebviewRef = (el, messengerId) => {
    if (el && !el._chatcenterInit) {
      ...
      // v0.86.5 DIAG: смена чата внутри WebView (Telegram Web меняет hash при клике на чат)
      addListener('did-navigate-in-page', (e) => {
        try {
          const url = e?.url || ''
          const hash = url.split('#')[1] || ''
          traceNotif('nav', 'info', messengerId, hash.slice(0, 120), `inpage url=${url.slice(0, 80)}`)
          // v0.86.5 DIAG: через 600мс после навигации снимаем геометрию и видимость WebView и его родителя
          setTimeout(() => {
            try {
              ...
              // Найти что лежит в центре WebView — определит overlay
              let topAtCenter = ''
              try {
          ...
          // v0.86.5 DIAG: опрашиваем САМ WebView через executeJavaScript — DOM/background/canvas/WebGL/Telegram-селекторы
          setTimeout(() => {
            try {
              const diagScript = `(function(){
                try {
                  ...
                  P('webgl',(function(){try{var c=document.createElement('canvas');var g=c.getContext('webgl')||c.getContext('experimental-webgl');return !!g?('ok renderer='+(g.getParameter(g.UNMASKED_RENDERER_WEBGL||0x9246)||'?')).slice(0,80):'none'}catch(e){return 'err:'+e.message.slice(0,40)}})());
      ...
      // v0.86.5 DIAG: фрейм упал (вложения, media, iframe чата)
      addListener('did-frame-finish-load', (e) => {
        if (e?.isMainFrame === false) {
          traceNotif('frame', 'info', messengerId, '', `subframe finished`)
        }
      })

      // v0.86.5 DIAG: устанавливаем глобальный ловец ошибок в WebView при загрузке страницы
