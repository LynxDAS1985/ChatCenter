...
export function buildTabContextMenuItems({ isNative = false, pinned = false } = {}) {
  return [
    ...(!isNative ? [
      ...
      // v1.2.440: ручное переснятие фото аккаунта. Нужно, потому что снимок кэшируется в хранилище
      // страницы мессенджера и сам обновляется лишь раз в сутки; если там оказалось ЧУЖОЕ фото
      // (брали аватар собеседника из открытого чата), ждать сутки неудобно. Только для веб-страниц:
      // у нативного «Общего чата» фото берётся из API, кэша страницы нет → пункт бессмыслен.
      { action: 'refreshAvatar', icon: '🖼', label: 'Обновить фото аккаунта' },
    ] : []),
