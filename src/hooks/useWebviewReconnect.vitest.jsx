...
describe('Экран «Нет связи»', () => {
  // v1.2.469: ВРЕМЯ ЗАМОРОЖЕНО. Раньше здесь стояло `const now = Date.now()` — оно бралось ОДИН раз
  // при загрузке файла, а сам экран сравнивает его с ТЕКУЩИМ временем и пересчитывает отсчёт раз в
  // секунду. Под нагрузкой полного прогона между загрузкой файла и выполнением проверки успевала
  // пройти секунда с лишним, отсчёт показывал 4 вместо 5 — и проверка падала «через раз»
  // (2026-09-16: упала в полном прогоне, но 5 из 5 зелёных в отдельном запуске).
  //
  // Лечение то же, что в блоке «Переподключение — поведение» выше: поддельные часы. Теперь время
  // ОДНО и то же и в тесте, и в экране, а проверка «первая ступень = 5 секунд» сохраняет смысл —
  // в отличие от ослабления до «любая цифра».
  //
