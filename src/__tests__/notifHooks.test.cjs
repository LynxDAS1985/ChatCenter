...
test('MAX: sidebar шлёт ribbon только при росте unread badge', () => assert(maxCode.includes('function _maxUnreadInfo') && maxCode.includes('unreadIncreased') && maxCode.includes('skip no unread increase') && maxCode.includes("src: 'max-sidebar', u: info.unread"), 'MAX sidebar должен требовать рост unread, а не только изменение preview'))
