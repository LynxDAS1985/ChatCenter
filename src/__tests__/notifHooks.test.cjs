...
test('MAX: sidebar decision диагностика пишет полные поля без slice', () => assert(maxCode.includes('__CC_DIAG__max-sidebar-decision') && maxCode.includes('prevBody:prevBody') && maxCode.includes('stableUnreadBodyChanged:stableUnreadBodyChanged') && maxCode.includes('freshTitleMs:freshTitleMs') && !maxCode.includes('sender.slice'), 'MAX sidebar должен писать полный decision JSON для разбора первого сообщения'))
