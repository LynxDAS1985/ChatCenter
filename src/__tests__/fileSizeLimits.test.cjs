...
test('Общий renderer код (src/ без тестов) < 35030 строк (сейчас ' + totalSrc + ')', function () {
  assert(totalSrc < 35030, totalSrc + " > 35030")  // v1.2.315-316: логотипы ВК/МАКС; v1.2.321: +пункт Ozon в каталоге; v1.2.330: +ozonNavigate.js; v1.2.333: логотип Ozon (ключ+ветка+импорт); v1.2.349: счётчики Ozon (ozonCounts + проводка + логи виджета); v1.2.350: тихая ловля ERR_ABORTED в виджете
})
