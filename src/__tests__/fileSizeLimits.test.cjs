...
test('Общий renderer код (src/ без тестов) < 35090 строк (сейчас ' + totalSrc + ')', function () {
  assert(totalSrc < 35090, totalSrc + " > 35090")  // v1.2.315-316: логотипы ВК/МАКС; v1.2.321: +пункт Ozon в каталоге; v1.2.330: +ozonNavigate.js; v1.2.333: логотип Ozon (ключ+ветка+импорт); v1.2.349: счётчики Ozon (ozonCounts + проводка + логи виджета); v1.2.350: тихая ловля ERR_ABORTED в виджете; v1.2.376: само-освежение фоновой «Вопросы» (ozonBgWatcher +reload-таймер)
})
