...
// v1.0.1: лимит 25200 → 26000 — useAppCounters hook + PanelModal обёртка + 3 кнопки в TabBar + 3 state/handler в App.jsx (Phase 4 UI интеграция).
...
test('Общий renderer код (src/ без тестов) < 26000 строк (сейчас ' + totalSrc + ')', function () {
  assert(totalSrc < 26000, totalSrc + ' > 26000')
})
