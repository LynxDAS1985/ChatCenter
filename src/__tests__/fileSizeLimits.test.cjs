...
// v1.2.203: лимит 32600 → 32760 — НЕСКОЛЬКО фото в окне (лента миниатюр, переключение, поворот у
// каждой, верхняя панель кнопок, границы фото). Крупная UI-фича, неустранима в renderer.
// v1.2.207: лимит 32760 → 32960 — окно фото: 7 функций (без сжатия, порядок мышкой, прогресс,
// размер МБ, удалить текущее, зум Вписать/1:1, подтверждение закрытия). Чистая логика вынесена в
// photoSendUtils.js; UI неустраним в renderer.
// v1.2.209-210: лимит 32960 → 33020 — счётчик подписи (лимит 1024) + «текст отдельным сообщением»
