...
// v1.0.1: лимит 25200 → 26200 — useAppCounters hook + PanelModal обёртка + 3 кнопки в TabBar + 3 state/handler в App.jsx (Phase 4 UI интеграция).
...
test('Общий renderer код (src/ без тестов) < 26200 строк (сейчас ' + totalSrc + ')', function () {
  assert(totalSrc < 26200, totalSrc + ' > 26200')
})
