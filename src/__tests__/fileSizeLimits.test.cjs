...
// v1.2.0-alpha.1 (Этап 1 AI Bridge): лимит 26400 → 27200 — aiBridge/contracts.js (~95 строк; в v1.2.468 переехал в shared/aiBridgeContracts.js) +
...
// v1.2.130: лимит 31150 → 31200 — Вариант 1 строки чата: время последнего сообщения
...
// v1.2.138: лимит 31270 → 31450 — локальное закрепление чатов (📌). Чистая логика
...
test('Общий renderer код (src/ без тестов) < 31150 строк (сейчас ' + totalSrc + ')', function () {
  assert(totalSrc < 31150, totalSrc + " > 31150")  // v1.2.495: планка ОПУЩЕНА 31200 -> 31150 (-50, факт 31104). Файл `src/utils/consoleMessageParser.js` (92 строки) переехал в `shared/consoleMessageParser.js`: это ЧИСТЫЙ разбор строк `__CC_*`, прилетающих из ЧУЖИХ страниц (window/document/React не трогает) — по правилу ADR-044 обособленный модуль, а не код интерфейса. Планка опущена сразу, чтобы освободившиеся строки не заросли молча; запас 46 строк.  // v1.2.492-493: планка 31270 -> 31200 после выноса autoReplyStats в shared/ (факт 31195).  // v1.2.485: планка ОПУЩЕНА 34340 -> 31270 (-3040, факт 31256). Из бюджета вынесены ОДИННАДЦАТЬ обособленных модулей, которые не являются кодом интерфейса (правило ADR-044: shared/ = «вне бюджета renderer, обособленный модуль или данные»): скрипты для ЧУЖИХ страниц (maxTitleFallbackScript, webviewDiagnostics, messengerPresets с accountScript), данные и определитель мессенджеров (messengerConfigs), чистая логика без React (diagnosticsAnalysis, diagnosticsSession, diagnosticsTargets, connectionHealth, webviewHealthProbe, webviewBlackWatchdog, webContentsViewBridge, messageProcessing, rulesImportExport) и конфиги ИИ-вкладок (aiWebviewConfigs, aiWebviewContext, aiWebviewDiagnostics). Планка опущена сразу, чтобы освободившиеся 3040 строк не заросли молча. // v1.2.478: планка ОПУЩЕНА 34540 -> 34340 (-200). Файл `src/utils/webAvatarScript.js` (230 строк) переехал в `shared/webAvatarScript.js`: это ТЕКСТ скрипта для ЧУЖОЙ страницы, а не код интерфейса — в бюджете интерфейса ему не место. Прежняя запись ниже («впрыскиваемые строки нельзя вынести в shared/») ОШИБОЧНА и опровергнута фактом: `shared/browserBannerHider.js` (v1.2.434) — такой же впрыск и живёт в shared/, а соседний `shared/webAvatarGate.js` уже лежал там. Планка опущена сразу, чтобы освободившиеся строки не заросли молча (ADR-044). // v1.2.468: планка ОПУЩЕНА 34640 -> 34540 (-100). Файл `src/utils/aiBridge/contracts.js` (135 строк, из них одна строка кода — остальное описания типов) переехал в `shared/aiBridgeContracts.js`: его тянули ШЕСТЬ файлов ГЛАВНОГО процесса (main/ai/bridge/*, main/handlers/aiBridgeIpcHandlers.js), то есть главный процесс лез за общим файлом ВНИЗ, в папку интерфейса — прямое нарушение ADR-050. Планку опустили сразу, чтобы освободившиеся 140 строк не заросли молча (правило ADR-044). Запас после переноса — 40 строк. // v1.2.448: планка ОПУЩЕНА 35540 -> 34640 (-900). Папка src/shared/ переехала в корневую shared/ (notificationSource.js + вся подпапка tools/ — 918 строк без тестов). Это ОБЩИЙ код: им пользуется и главный процесс (main/ai/aiAgentSetup.js, main/handlers/aiToolIpcHandlers.js лезли в src/ снизу вверх), поэтому в бюджете интерфейса ему было не место. Планка опущена сразу, чтобы освободившееся место не заросло молча — правило ADR-044. // v1.2.447: 35480 -> 35540 (+60, факт 35494) — полоса «нет связи» для «Общего чата» (NativeConnectionStrip 88 строк + 4 строки проводки). Поднятие законно по ADR-044 (запрещено поднимать ВМЕСТО разгрузки), и разгрузка в этой же версии СДЕЛАНА: 28 строк ушли из src/utils/webviewSetup.js в shared/webviewPageFixups.js, а вся чистая логика полосы — в shared/connectionStripState.js (108 строк, вне бюджета). ОСТАВШИЙСЯ КАНДИДАТ НА РАЗГРУЗКУ (не делал — структурная правка, нужна отдельная задача): папка src/shared/ (notificationSource.js) лежит ВНУТРИ бюджета, хотя это общий модуль — её место в корневой shared/. // v1.2.445: 35300 -> 35480 (+180, факт 35428) — НОВАЯ функция: экран «Нет связи» (WebviewOfflineOverlay 136 строк) + проводка переподключения (useWebviewReconnect 130). Вся ЧИСТАЯ логика уже вынесена в shared/reconnectPlan.js (193 строки, вне бюджета) — поднимать планку тут законно по ADR-044: она запрещает поднятие ВМЕСТО разгрузки, а разгружать больше нечего. v1.2.442: планка ОПУЩЕНА 36090 -> 35300 (было 36088, стало 35172): данные «Что нового» (changelogData + архив, 917 строк) переехали в shared/ — они растут с каждой версией и в бюджете renderer им не место. Запас 128 строк. Ниже — история прежних поднятий. v1.2.424: подавление фантомов МАКС на общем узле handleNewMessage (openChat-флаг + комментарий-ловушка про document.hidden). v1.2.416: реальный 1px-пинок раскладки веб-вкладки при активации (Ловушка 64 — лечит чёрный экран МАКС). v1.2.406: единый экран загрузки (скелет+шапка). v1.2.403: заставка чатов по макету (рост счётчика + проявление аватарок + прогресс-полоса) + выравнивание раннего экрана. v1.2.400: живая заставка-заглушка NativeAppFallback (кружки). v1.2.398: заставка чатов — min-время видимости + лог показа/скрытия. v1.2.394: фиксы заставки по ревью (loadDone-сохранение «итога» + pointer-events + логи). v1.2.393: заставка первой загрузки списка чатов (ChatListLoadingSplash + флаг chatsFirstLoadDone + проводка). v1.2.392: ozonBgWatcher rv-стартовое (Вариант C). v1.2.385: записи «Что нового» v1.2.383-385 (changelogData). v1.2.381: pickNotifIconDataUrl (аватар>логотип, фикс регресса ВК). v1.2.315-316: логотипы ВК/МАКС; v1.2.321: +пункт Ozon в каталоге; v1.2.330: +ozonNavigate.js; v1.2.333: логотип Ozon (ключ+ветка+импорт); v1.2.349: счётчики Ozon (ozonCounts + проводка + логи виджета); v1.2.350: тихая ловля ERR_ABORTED в виджете; v1.2.376: само-освежение фоновой «Вопросы» (ozonBgWatcher +reload-таймер); v1.2.379: resolveMessengerLogo (логотип уведомления по типу) + логотип в авто-ответе
})
