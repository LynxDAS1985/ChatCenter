...
// ──────────────────────────────────────────────────────────────────
// v0.89.45: запрет на возвращение deprecated BrowserView
// По Electron docs (https://www.electronjs.org/docs/latest/api/browser-view):
// «The BrowserView class is deprecated, and replaced by the new WebContentsView class».
// Действует с Electron v29.0.0. Не должно появиться в main/ ни в каком виде.
// ──────────────────────────────────────────────────────────────────
...
test('main/: deprecated BrowserView не используется (Electron v29+ → WebContentsView)', () => {
  const mainFiles = listFilesRecursive(path.resolve(process.cwd(), 'main'))
  const offenders = []
  for (const f of mainFiles) {
    const raw = fs.readFileSync(f, 'utf8')
    // Снимаем комментарии чтобы не падать на упоминаниях в /* deprecated since v29 */
    const code = raw
      .replace(/\/\/[^\n]*/g, '')
      .replace(/\/\*[\s\S]*?\*\//g, '')
    ...
    // require('electron').BrowserView или import { BrowserView } from 'electron'
    ...
    // const { BrowserView } = require('electron') — обработаем destructuring
