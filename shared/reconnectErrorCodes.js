...
/**
 * v1.2.498 (находка ревью #11): предел РАСТЁТ с каждой попыткой — 45 → 90 → 135 → 180 с (потолок).
 *
 * ЗАЧЕМ: постоянные 45 с на медленном канале давали петлю — честная загрузка не успевала, мы её
 * бросали, начинали заново и снова не успевали. Растущий предел даёт медленной связи шанс, а
 * зависшую наглухо страницу всё равно отпускает (первая попытка — те же 45 с).
 */
export const ATTEMPT_TIMEOUT_MAX_MS = 180000

/** Предел ожидания для попытки номер N (1, 2, 3 …). */
export function attemptTimeoutFor(attempt) {
  const n = Math.max(1, Number(attempt) || 1)
  return Math.min(ATTEMPT_TIMEOUT_MS * n, ATTEMPT_TIMEOUT_MAX_MS)
}

/**
 * v1.2.498: предел ожидания ПРОБЫ «жива ли страница» (находка ревью #4). Раньше предел стоял только
 * на загрузке, и зависшая проба оставляла запись в фазе «идёт» навсегда — та самая беда, ради
 * которой всё делалось, только другим путём.
 */
